...
int main (int argc, char *argv[])
{
    ...
    if(argc!=2) {
        printf("Please give 1 argument: rootname (no extension  nor postfix) of input JPG and conft file.\n");
        printf("Basically the program will conern it self with adding \".jpg\" and \".conft\" to it and both thosse files need to of course exist\n");
        exit(EXIT_FAILURE);
    }
    int i;
    ...
    sprintf(outname, "%.*s_imdm.png", (int)(dot-argv[1]), argv[1]);
    ...
    ExifData *ed = exif_data_new_from_file(ijpg);
    if (!ed) {
    ...
    strcat(tstr, "Imdetailing conft point for:\n");
